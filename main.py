...
async def handle_text(message: Message):
    ...
    # Команды рулетки
        ...
        if message.chat.type == "private":
            await message.reply("🚫 Рулетку можно играть только в чатах!\n\n🎮 Перейдите в чат чтобы сделать ставку")
            return
        ...
        try:
            ...
            if len(parts) >= 3:
                ...
                if user:
                    ...
                    if bet is not None:
                        if bet <= user['balance'] and bet > 0:
                            # Объединяем все что после ставки в одну строку и парсим ставки
                            bet_args = " ".join(parts[2:])
                            bet_values = []
                            
                            # Разделяем по пробелам, но сохраняем диапазоны
                            current_bet = ""
                            for word in bet_args.split():
                                if "-" in word and current_bet == "":
                                    # Это диапазон
                                    bet_values.append(word)
                                elif "-" in current_bet:
                                    # Продолжение диапазона (не должно быть)
                                    current_bet += " " + word
                                    bet_values.append(current_bet)
                                    current_bet = ""
                                else:
                                    # Обычная ставка или начало диапазона
                                    if current_bet:
                                        bet_values.append(current_bet)
                                    current_bet = word
                            
                            # Добавляем последнюю ставку если есть
                            if current_bet:
                                bet_values.append(current_bet)
                            
                            # Отладка - покажем что получилось
                            print(f"🔍 DEBUG: bet_args='{bet_args}', bet_values={bet_values}")
                ...
                await message.reply("❌ Использование: Рулетка/РУЛ [ставка] [ставки...]\n\n📋 Примеры:\n• Рулетка 1000 7\n• РУЛ 1000 к (красный)\n• РУЛ 1000 ч (черный)\n• Рулетка 1000 17-27\n• РУЛ 1000 odd\n• РУЛ 1000 1 3 5 12-16 17-19 27", reply_to_message_id=message.message_id)
        except (ValueError, IndexError):
            await message.reply("❌ Использование: Рулетка/РУЛ [ставка] [ставки...]\n\n📋 Примеры:\n• Рулетка 1000 7\n• РУЛ 1000 к (красный)\n• РУЛ 1000 ч (черный)\n• Рулетка 1000 17-27\n• РУЛ 1000 odd\n• РУЛ 1000 1 3 5 12-16 17-19 27", reply_to_message_id=message.message_id)
    elif text == "го":
        if message.chat.type == "private":
            await message.reply("🚫 Рулетку можно играть только в чатах!\n\n🎮 Перейдите в чат чтобы запустить рулетку")
            return
        ...
        if message.chat.type == "private":
            await message.reply("🚫 Историю рулетки можно смотреть только в чатах!")
            return
            
        await roulette_game.show_history(message)
    elif text == "рулетка" or text == "рул":
...
async def handle_game_control(message: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=f"🎰 Рулетка {'✅' if settings.get('roulette', True) else '🚫'}", 
...
async def game_control_callback(callback: types.CallbackQuery):
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    text=f"🎰 Рулетка {'✅' if updated_settings.get('roulette', True) else '🚫'}", 
                    callback_data="game_roulette"
                )
            ],
            [
...
🎡 Вилин - колесо фортуны
🎰 Рулетка/РУЛ [ставка] [ставки...] - классическая рулетка (только в чатах)
