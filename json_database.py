...
class JSONDatabase:
    ...
    async def load_data(self):
        ...
        try:
            ...
            self.data = {"users": {}, "settings": {"trade": True, "mines": True, "slots": True, "wheel": True, "roulette": True}}
    ...
    async def update_profile_visibility(self, user_id: int, is_closed: bool):
        """Обновить видимость профиля"""
        async with self._lock:
            user_id_str = str(user_id)
            if user_id_str in self.data["users"]:
                self.data["users"][user_id_str]["profile_closed"] = is_closed
                await self.save_data()
    
    async def add_game_history(self, user_id: int, game: str, bet: int, result: str, win_amount: int = 0):
        ...
        async with self._lock:
            ...
            if user_id_str in self.data["users"]:
                ...
                self.data["users"][user_id_str]["game_history"].insert(0, history_entry)
                
                # Оставляем только последние 10 записей
                if len(self.data["users"][user_id_str]["game_history"]) > 10:
                    self.data["users"][user_id_str]["game_history"] = self.data["users"][user_id_str]["game_history"][:10]
                
                await self.save_data()
    
    async def get_game_history(self, user_id: int) -> list:
        """Получить историю игр пользователя"""
        async with self._lock:
            user_id_str = str(user_id)
            if user_id_str in self.data["users"]:
                return self.data["users"][user_id_str].get("game_history", [])
            return []
